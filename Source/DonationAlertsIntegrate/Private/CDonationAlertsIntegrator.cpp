...
void UCDonationAlertsIntegrator::Disconnect()
{
	if (HttpRouter.IsValid())
	{
		if (Callback.IsValid())
		{
			HttpRouter->UnbindRoute(Callback);
		}

		if (URLReceived.IsValid())
		{
			HttpRouter->UnbindRoute(URLReceived);
		}
	}
...
ECurrencyType UCDonationAlertsIntegrator::GetCurrencyType(const FString CurrencyText)
{
	...
	if(CurrencyText == "RUB")
	{
		return ECurrencyType::ECT_RUB;
	}
	if(CurrencyText == "EUR")
	{
		return ECurrencyType::ECT_EUR;
	}
	if(CurrencyText == "USD")
	{
		return ECurrencyType::ECT_USD;
	}
	if(CurrencyText == "UAH")
	{
		return ECurrencyType::ECT_UAH;
	}
	if(CurrencyText == "BYN")
	{
		return ECurrencyType::ECT_BYN;
	}
	if(CurrencyText == "KZT")
	{
		return ECurrencyType::ECT_KZT;
	}
	if(CurrencyText == "BRL")
	{
		return ECurrencyType::ECT_BRL;
	}
	if(CurrencyText == "TRY")
	{
		return ECurrencyType::ECT_TRY;
	}
	if(CurrencyText == "PLN")
	{
		return ECurrencyType::ECT_PLN;
	}
	return ECurrencyType::ECT_MAX;
}